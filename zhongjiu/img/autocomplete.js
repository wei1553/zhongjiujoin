...
var searchBaseUrl = "http://www.zhongjiu.cn/search.htm";
...
function search(obj) {
    ...
    if (url != undefined && url != '') {
        ...
        if (ad != null && ad.KeywordId > 0 && ad.Title == val) {
            var param = { keywords: ad.Keyword, keywordId: ad.KeywordId };
            url = baseUrl + $.param(param);
            window.location = url;
        } else {
            url = baseUrl + $.param({ keywords: val });
            window.location = url;
        }
    }
}
...
function fialsearch(obj) {
    ...
    var baseUrl = searchBaseUrl + "?";
    var url = baseUrl + $.param({ keywords: val });
    window.location = url;
...
$(function () {
    ...
    var keywords = new Bloodhound({
        ...
        remote: {
            ...
            url: 'http://www.zhongjiu.cn/search/SearchAdNameList?keyword=%QUERY&callback=?',
            ajax: { dataType: 'jsonp' },
    ...
    $('#searchBox').typeahead({ autoselect: false, minLength: 0, hint: false }, {
        displayKey: 'Name',
    ...
    */
    $('#searchBox').on("typeahead:cursorchanged", function (e, o, n) {
        var me = $(this);
        ...
        if (o.ResetUrl != '') {
            ...
            var param = { keywords: o.Name, keywordId: o.Id };
            if (data.prevkeyword != '') {
    ...
    $('#searchBox').on("typeahead:selected", function (e, o, n) {
        var me = $(this);
        var data = me.data();
        if (o.ResetUrl != '') {
    ...
    if (parseUri(window.location).path.toLowerCase() != "/search/index") {
        var keyword = $("#searchBox");
        if (keyword) {
//            $.getJSON('http://s.zhongjiu.cn/search/handler.ashx?action=ad&callback=?', function (result) {
//                if (result != null) {

//                    var btn = $("#btn-search");
//                    keyword.val(result.Title).focus(function () {
//                        this.value = '';
//                        btn.data('url', '');
//                    }).blur(function () {
//                        var val = this.value;
